...
document.addEventListener("DOMContentLoaded", function () {
    console.log("THREADVERSE order success page started!");
...
function loadOrderSuccess() {
    ...
    try {
        const savedOrder = localStorage.getItem("threadverseLastOrder");
    ...
    if (!lastOrder) {
        showOrderUnavailable(urlOrderId);
        return;
    }
    ...
    const customerDetailsElement =
        document.getElementById("success-customer-details");
    ...
    const deliveryDetailsElement =
        document.getElementById("success-delivery-details");
    ...
    if (deliveryDetailsElement) {
        ...
        deliveryDetailsElement.innerHTML = `
            <div class="success-address">
                ...
                <strong>${escapeHTML(
                    lastOrder.customerName || "THREADVERSE Customer"
                )}</strong>

                <p>${escapeHTML(
                    lastOrder.address || "Address not available"
                )}</p>

                <p>
                    ${escapeHTML(lastOrder.city || "")}
                    ${lastOrder.city && lastOrder.state ? "," : ""}
                    ${escapeHTML(lastOrder.state || "")}
                </p>

                <p>
                    PIN Code: ${escapeHTML(
                        lastOrder.pincode || "Not available"
                    )}
                </p>
    ...
    const orderItemsElement =
        document.getElementById("success-order-items");
    ...
    const totalElement =
        document.getElementById("success-order-total");
    ...
    if (customerDetailsElement) {
        customerDetailsElement.innerHTML = `
            <p class="success-unavailable-message">
                ...
                Please return to THREADVERSE and continue shopping.
            </p>
        `;
    }
